...
**Note:** We will be using powershell related commands. If you are using linux, check this file [Linux Velero Guide](./linux-velero-guide.md). . Also, we are using Azure Kubernetes Service for backup with Velero.

## Instal Velero CLI

Let's start simply by installing Velero CLI on our windows machine.
...
```
Invoke-WebRequest `
  -Uri "https://github.com/vmware-tanzu/velero/releases/download/v1.5.1/velero-v1.5.1-windows-amd64.tar.gz" `
  -OutFile "$env:TEMP\velero.tar.gz"
```
Extract the Velero Folder
```
tar -C $env:TEMP -xvf "$env:TEMP\velero.tar.gz"
```
Move the Binary Files
```
New-Item -ItemType Directory -Force -Path "C:\Program Files\Velero"

Move-Item `
  "$env:TEMP\velero-v1.5.1-windows-amd64\velero.exe" `
  "C:\Program Files\Velero\velero.exe"
```
...
```
$env:AZURE_BACKUP_RESOURCE_GROUP = "<backup-resource-group>"
$env:AZURE_STORAGE_ACCOUNT_NAME = "<storage-account-name>"
$env:BLOB_CONTAINER = "<blob-container-name>"
$env:AZURE_BACKUP_SUBSCRIPTION_ID = "<azure-subscription-id>"
```
...
```
@"
AZURE_STORAGE_ACCOUNT_ACCESS_KEY=$env:AZURE_STORAGE_ACCOUNT_ACCESS_KEY
AZURE_CLOUD_NAME=AzurePublicCloud
"@ | Out-File "$env:TEMP\credentials-velero" -Encoding ascii
```
